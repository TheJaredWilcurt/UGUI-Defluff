...
$(document).ready(function () {
    ...
    fs.copy(path.join(appLocation, 'truepng.exe'), path.join(appData, 'truepng.exe'));
     ...
     * @param  {string} nameID 'cat__859196'
     */
    function truepng (file, name, nameID) {
        var copyOfOrig = path.join(appData, 'temp', nameID + '.png');
        fs.copy(file, copyOfOrig, function () {
            process.chdir(appData);
            var exec = require("child_process").exec;
            var input = '"' + path.join('temp', nameID) + '.png"';
            var executableAndArgs = 'truepng ' + input + ' /o4';
    ...
    function onFilesDrop (files) {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            if (file.name.endsWith('.png')) {
                ...
                showFTUX(false);
                truepng(file.path, file.name, nameID);
            }
        };
    }
