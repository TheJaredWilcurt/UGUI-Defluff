...
$(document).ready(function () {
    ...
    fs.copy(path.join(appLocation, 'zopfli.exe'), path.join(appData, 'zopfli.exe'));
    fs.copy(path.join(appLocation, 'zopflipng.exe'), path.join(appData, 'zopflipng.exe'));
    ...
    function replaceOrRetain (file, name, nameID) {
        ...
        var original = path.join('temp', name + '.png');
                  ...
                  (sizeSetting == 'always')
                    ...
                    if (retentionSetting == 'replace') {
                        fs.copy(tempFileFullPath, file, { 'clobber': true }, function () {
                            ...
                            ugui.helpers.deleteAFile(path.join(appData, original));
                        });
     ...
     * @param  {string} nameID 'cat__859196'
     */
    function zopfli (file, name, nameID) {
        ...
        fs.copy(file, copyOfOrig, function () {
            ...
            var input = '"' + path.join('temp', name) + '.png"';
    ...
    function onFilesDrop (files) {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            if (file.name.endsWith('.png')) {
                ...
                showFTUX(false);
                zopfli(file.path, file.name, nameID);
            }
        };
    }
